...
public class ArrayExample1 {
	...
	public void ex1( ) {
		...
		//new int[3] : heap 영역에 int3칸자리 int[] 을 생성(할당)
		            //** 생성된 int[]에는 시작 주소가 지정된다!! **
		//arr = new int[3];
		//(int[])  (int[])  -> 같은 자료형 == 연산 가능
		
//		int[] arr2 = new double[5];  //자료형이 다르면 연산이 안된다!
		...
		System.out.println("arr의 주소값? : " + arr); // [I@71be98f5
		
		//배열 요소 값 대입
		//arr은 int[] 참조형 변수 이지만
		//arr[0], arr[1] , arr[2]은 int 자료형 변수이기 떄문에 정수값을 대입할 수 있다!
	...
	public void ex2() {
		...
		//배열과 for문
		for(int i=0; i < arr.length; i++) {  // 인덱스는 3번까지 있기떄문에 미만으로 하지않고 이하로 설정하면 4번인덱스까지 접근하려고 함
			// 0  1  2 3
	...
	public void ex3() {
		...
		//height에 heap영역에 새로 생성된 double 5칸짜리 double[]의 시작 주소를 대입
		
		for(int i = 0; i < height.length; i++) {
			
			System.out.print((i+1) + "번 키 입력 : ");  // 1 2 3 4 5 i가 0부터 이기떄문에 +1해줌
	...
	public void ex4() {
		...
		System.out.printf("\n합계 : %d " , sum);
		System.out.printf("\n평균 : %.1f " , (double)(sum /score.length));  //person
		System.out.printf("\n최고점 : %d " , max);
		System.out.printf("\n최저점 : %d " , min);
	...
	public void ex8() {
		...
		for(int i =0; i < arr.length; i++) {
			...
			if(arr[i].equals(input)) {  //String 비교시 equals() 메서드 사용
				System.out.println(i + "번째 인덱스에 존재");
				flag = true;
			}
