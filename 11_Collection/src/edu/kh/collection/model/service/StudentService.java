...
public class StudentService {
	...
	//java.util.List 인터페이스 : List 반드시 필요한 필수 기능을 모아둔 인터페이스
	//*인터페이스 객체 생성 x , 부모 참조변수 o
	
	//java.util.ArrayList : 배열 형태 List(가장 대표적인 List 자식 클래스)
	
	//ArrayList() 기본 생성자 : 기본 크기 10짜리 리스트생성
	//                          -> 하지만 리스트는 크기가 늘었다 줄었다 하기떄문에 큰의미 없음.
...
//	private List <Student> studentList = new ArrayList<Student>() ; //검색(조회)에 효율적
	...
	public StudentService() { //기본생성자
		...
		studentList.add(new Student("개굴이",22,"경기도 고양시",'F',100));
		studentList.add(new Student("강아지",35,"경기도 안산시",'M',60));
		studentList.add(new Student("김나나",27,"서울시 성북구",'F',70));
		studentList.add(new Student("고영희",24,"서울시 서대문구",'F',80));
...
//		System.out.println(studentList.get(3));
	...
	public void selectAll() {
			// -List 에 저장된 데이터의 개수를 얻어오는 방법 : int Lsidt.size()
			// -> 배열명.length 대신 사용
		
		System.out.println("=======학생 전체 조회========");
		
		//studnetList가 비어있는 경우 "학생 정보가 없습니다" 출력
		
//		if(studentList.size() == 0)
		if(studentList.isEmpty()) { //컬렉션이 비어있다면 true반환
			System.out.println("학생 정보가 없습니다");
			return; //현재 메소드를 종료하고 호출할 곳으로 돌아감.
			        // 단, 반환값은 없다(void)
		}
...
//		
		...
		//작성법
		//for (컬렉션 또는 배열에서 꺼낸 한개의 요소를 저장 할 변수 : 컬렉션명 또는 배열명){}
	 ...
	 * - studentLsit 범위 내 인덱스 번호인지 검사, 초과했다면 "범위를 넘어선 값을 입력할 수 없습니다" 문자열 반환
	 * - 수정이 완료되었다면, "000의 정보가 변경되었습니다" 문자열 반환
	 */
	public String updateStudent() throws InputMismatchException {
		
		// List.set(int index , Studnet e(element))
		// -> List의 index번째 요소를 전달받은 e로 변경
		// -> 반환값 Student== 변경 전 Studnet 객체가 담겨있다.
		...
		//3) studentLsit 범위 내 인덱스 번호인지 검사, 초과했다면 "범위를 넘어선 값을 입력할 수 없습니다" 문자열 반환
		}else if(index >= studentList.size()) {
			return "범위를 넘어선 값을 입력할 수 없습니다";
			
		//4)수정이 완료되었다면, "000의 정보가 변경되었습니다" 문자열 반환
		} else {
			//수정 코드 작성
			...
			// 입력받은 inddex번째에 새로운 학생 정보를 세팅 == 수정
			// index 번째에 있던 기존 학생 정보가 반환되고, 그 객체르 temp에 저장한다.
