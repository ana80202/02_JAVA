...
public class MemberService {
  ...
  // class는 속성과 기능을 가지고 있다.
  //속성
  private Scanner sc = new Scanner(System.in);

  //Member 5칸자리 객체배열 선언 및 할당
  ...
  public void displayMEnu( ) {
	  ...
	  do {
		  ...
		  case 1 : System.out.println( signUp()); break;   /*회원가입기능호출*/ 
		  case 2 : System.out.println(login()); break;   /*로그인기능호출*/ 
		  case 3 : System.out.println(searchMember()); break;   /*회원 정보 조회 기능호출*/
		  case 4 :  
  ...
  public int emptyIndex() {
	  ...
	  //-> for문에서 retrun되지 않았다 == 배열에 빈칸이 없다
	  return-1;
  ...
  public String signUp() {
	  ...
	  //객체 배열 memberArr에 강비한 회원 정보를 저장할 예정
    	  ...
    	  if(memberArr[i] != null) { //회원정보가 있을 경우
    		  ...
    		  if(memberArr[i].getMemberId().equals(memberId) &&
    		     memberArr[i].getMemberPw().equals(memberPw)) {
    			  ...
    			  break; //가장 가까운 반복문인 for문을 종료시킴 /더이상 같은 아이디, 비밀번호가 없기떄문에 for문 종료
    		  }
  ...
  public int updateMember() {
	  ...
	  //비밀번호 입력 받아서 로그인한 회원의 비밀번호와 일치하는지 확인
	  System.out.print("비밀번호 입력 : ");
	  String inputPw = sc.next();
	  
	  if(inputPw.equals(loginMember.getMemberPw())) {
		  //비밀번호가 같은 경우 로그인한 회원의 이름,나이,지역정보를
		  // 입력 받은 값으로 변경후 
		  //1 반환
