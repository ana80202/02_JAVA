...
public class ByteService {
	...
	/* ************************************* */
	//IO 관련된 코드는 IOException이 발생 될 가능성이 높다!
	...
	public void fileByteOutput() {
		...
		try {
			...
			//-> 2byte범주의 문자(영어, 숫자, 기본 특수문자 제외)는
			//   1byte 기반 스트림 통과 시
			//   데이터 손실이 발생해서 글자가 꺠지는 문제가 발생
//			
//			for(int i =0; i < content.length(); i++){
//				char ch = content.charAt(i); //i번쨰 글자 반환
...
//			
		...
		}finally { //try 예외 발생 여부 관꼐 없이 무조건 수행
...
BufferedOutputStream bos = null; //보조스트림 선언
		...
		try {
			...
			fos = new FileOutputStream("/io_test/20240222/바이트기반_테스트.Buffered.txt");
	...
	public void fileByteInput(){
		...
		try {
			...
			// -> 2byte 범주의 글자들이 꺠지는 문제가 발생
			
			//자바에서 byte는 정수형
			//-> 다루기 힘들기때문에 정수 기본형인 int로 변환해서 사용
