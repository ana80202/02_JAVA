...
public class ObjectService {
	 ...
	 * 
	 * -Java객체 (Instance == Object) 를 입/출력 할떄
	 * 사용하는 바이트 기반 "보조" 스트림
	 * (보조 스트림은 단독 사용 불가능!)
	 * 
	 * **직렬화(Serializable)
	...
	public void objectOutput() {
		...
		try {
			...
			oos = new ObjectOutputStream(fos); //객체 보조 스트림 생성한거
		...
		}catch(Exception e) { //Exception->프로그램 실행 중 발생할 수 있는
							  //정상적인 범위를 벗어난 경우에 처리를 하기 위한 방법
			...
			}catch(IOException e) { //IOException 입출력 작업 중에 발생하는 예외
									//입력과 출력을 다루는 메서드에 예외처리(IOException)이 없다면 컴파일 에러 발생
				e.printStackTrace();
			}
